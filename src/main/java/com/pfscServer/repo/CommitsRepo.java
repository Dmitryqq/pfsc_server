...
public interface CommitsRepo extends JpaRepository<Commit, Long>{ 
    ...
    @Query(joinQuery + " where upper(a.description) like upper(concat('%', ?1,'%')) order by b.activity desc,a.createDate desc")
    List<CommitDto> findByDescriptionContaining(String description);
    
    @Query(joinQuery + " where upper(a.description) like upper(concat('%', ?1,'%')) or a.createDate between ?2 and ?3 order by b.activity desc,a.createDate desc")
    List<CommitDto> findByDescriptionOrCreateDate(String param, LocalDateTime startDate, LocalDateTime endDate);
    
    @Query(joinQuery + " where a.userId = ?2 and upper(a.description) like upper(concat('%', ?1,'%')) order by b.activity desc,a.createDate desc")
    List<CommitDto> findByUserIdAndDescriptionContaining(String description, Long userId);
    
    @Query(joinQuery + " where a.userId = ?4 and (upper(a.description) like upper(concat('%', ?1,'%')) or a.createDate between ?2 and ?3) order by b.activity desc,a.createDate desc")
